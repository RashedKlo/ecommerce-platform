...
{
    public class GetCategoriesByFilterQuery
    {
        public static async Task<IEnumerable<CategoryDTO>> ExcuteAsync(ValueFilter valueFilter)
        {
            var Categories = new List<CategoryDTO>();
            int TotalCatgories = 0;
            int offset = (valueFilter.page.pageNumber - 1) * valueFilter.page.limitOfUsers;
            string query = @"Select CategoryID,Title ,Description,Image,CreatedAt,UpdatedAt,
        ...
        ORDER BY CategoryID OFFSET @Offset ROWS FETCH NEXT @Limit ROWS ONLY";
            ...
            using (SqlCommand command = new SqlCommand(query, connection))
            {
                ...
                command.Parameters.AddWithValue("@Title", valueFilter.value);
                try
                {
