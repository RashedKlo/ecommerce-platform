...
{
    public class GetLatestProductsQuery
    {
        public static async Task<IEnumerable<ProductDTO>> ExcuteAsync(PageFilter page)
        {
            var products = new
            List<ProductDTO>();
            int TotalProducts = 0;
            int offset = (page.pageNumber - 1) * page.limitOfUsers;
            string query = @"SELECT Products.ProductID, Products.ProductName, Products.CategoryID, Categories.Title, Products.Price, Products.Discount,
            ...
            (SELECT COUNT(ProductID) From Products)As TotalProducts
             ...
             ORDER BY Products.CreatedDate OFFSET @Offset ROWS FETCH NEXT @Limit ROWS ONLY";
