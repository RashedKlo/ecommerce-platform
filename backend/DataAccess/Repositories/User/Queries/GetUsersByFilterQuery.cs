...
{
    public class GetUsersByFilterQuery
    {
        ...
        public static async Task<IEnumerable<UserDTO>> ExcuteAsync(FilterType.User Type, ValueFilter filterDTO)
        {
            var usersList = new List<UserDTO>();
            int TotalUsers = 0;
            int offset = (filterDTO.page.pageNumber - 1) * filterDTO.page.limitOfUsers;
            string query = @"
               SELECT Users.UserID, Users.UserName,Users.Role, Users.Email, Users.BirthDate, Users.ProfilePicture, Users.CreatedAt,
               Users.UpdatedAt, Countries.CountryID, Countries.Country, (Select Count(Users.UserID)From Users) AS TotalUsers
               FROM Countries INNER JOIN
            ...
            if (Type == FilterType.User.UserName)
                query = query.Replace("FilterType", "UserName");
            else
                query = query.Replace("FilterType", "Email");

            using (SqlConnection connection = new SqlConnection(clsDataAccessSettings.connectionString))
            using (SqlCommand command = new SqlCommand(query, connection))
            {
                command.Parameters.AddWithValue("@Offset", offset);
                command.Parameters.AddWithValue("@Limit", filterDTO.page.limitOfUsers);



                command.Parameters.AddWithValue("@value", filterDTO.value);
                try
                {
