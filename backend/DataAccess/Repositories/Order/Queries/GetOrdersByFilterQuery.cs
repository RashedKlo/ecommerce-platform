...
namespace DataAccess.Repositories.Order.Queries
{
    ...
    {
        static string query = @"SELECT Orders.OrderID, Orders.UserID, Users.UserName, Orders.OrderDate,
        ...
        public static async Task<IEnumerable<OrderDTO>> ExcuteAsync(FilterType.Order Type, ValueFilter filter)
        {
            var Orders = new List<OrderDTO>();
            int TotalOrders = 0;
            int offset = (filter.page.pageNumber - 1) * filter.page.limitOfUsers;

            if (Type == FilterType.Order.OrderDate)
                query = query.Replace("Filter", " OrderDate = @value");
            else if (Type == FilterType.Order.OrderStatus)
                query = query.Replace("Filter", "OrderStatus = @value");
            else if (Type == FilterType.Order.UserName)
                query = query.Replace("Filter", " Users.UserName like @value+'%'");
            ...
            using (SqlCommand command = new SqlCommand(query, connection))
            {
                ...
                command.Parameters.AddWithValue("@Limit", filter.page.limitOfUsers);
                try
                {
